...
# The flow checkpoint is loaded only to keep the initialization/CLI path equal;
# negative embeddings are exact frozen-catalog lookups from uniform non-click IDs.
exec "${project_root}/../.venv/bin/python" "${project_root}/flow.py" rank-continuous \
  --flow-checkpoint "${flow_run}/best.pt" \
  --output "${project_root}/experiments/flow_rns/p1_uniform_frozen_seed1" \
  --batch-size 2048 \
  --learning-rate 0.001 \
